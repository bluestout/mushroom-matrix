    ...
    class ProductInfo extends HTMLElement {
          ...
          .then((responseText) => {
            ...
            setTimeout(() => {
              ...
              if (elementToFocus) {
                ...
                // Prevent scroll by saving and restoring position
      ...
      handleUpdateProductInfo(productUrl) {
        return (html) => {
          ...
          this.toggleVariantDirections(variant?.id);
          ...
          // Manually update product variant id in the product form
          const productVariantId = html.querySelector('.product-variant-id');
          productVariantId.value = variant.id;

          // Update price in the ATC button
      ...
      toggleVariantDirections(variantId) {
        // Find all variant divs and default div
        const variantDivs = document.querySelectorAll('.variant-div[data-variant-id]');
        const defaultDiv = document.querySelector('.default-div');

        // Hide all divs first
        variantDivs.forEach((div) => (div.style.display = 'none'));
        if (defaultDiv) defaultDiv.style.display = 'none';

        // Look for matching variant div
        const matchingVariantDiv = document.querySelector(`.variant-div[data-variant-id="${variantId}"]`);

        if (matchingVariantDiv) {
          // Show the matching variant div
          matchingVariantDiv.style.display = 'block';
        } else if (defaultDiv) {
          // Show default div if no variant match
